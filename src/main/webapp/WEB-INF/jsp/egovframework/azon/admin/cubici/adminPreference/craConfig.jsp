    ...
    $("#itemSelect").change(function(){
    	...
    	for(var i=0; i<prizmEvalList.length; i++){
    		...
    		if(getData.SUBJECT_NO === subjectVal && getData.ITEM_NO === itemVal){
    			// 지표정의, 가중치는 데이터라 초행만 입력하면 됨 (신용평가 평가점수는 스코어가 0)
    			$('#ITEM_DEFINITION').prop('readonly', false);
    			$('#ITEM_WEIGHT').prop('readonly', false);
    			if(getData.SUBJECT_NO == 1){
...
function ratioUpdate(){
	...
	if($("#ITEM_DEFINITION").val() === "" || $("#ITEM_DEFINITION").val() === null || $("#ITEM_DEFINITION").val() === undefined){
		$("#modalAlertText").html("지표정의가 설정되지 않았습니다.");
	...
	$.ajax({
		url: "/admin/cubici/prizmEvalUpdate",
		type: "post",
		dataType: "json",
		data: params,
		success: function (result) {
			if(result.resultCode == 0){
				...
				alert("CRA 세부지표 업데이트 중 오류가 발생했습니다.");
			}
		},
...
function updDetail(UPD_SEQ){
	...
	$.ajax({
		url: "/admin/cubici/prizmEvalUpdList",
		type: "post",
		dataType: "json",
		data: param,
		success: function (result) {
			if(result.resultCode == 0){
				...
				alert("CRA 세부지표 수정내역 요청 중 오류가 발생했습니다.");
			}
		},
		error: function(result){
			alert("서버 통신 오류");
...
<div class="m-tab">
    <ul>
        ...
        <li class="active"><a href="/admin/cubici/adminPreference/craConfig">CRA Index</a></li>
