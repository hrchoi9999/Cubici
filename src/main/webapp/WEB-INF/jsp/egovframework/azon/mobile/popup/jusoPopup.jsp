...
<% 
	...
	String confmKey = "U01TX0FVVEgyMDIxMDYxNzE1NDQyNzExMTI5NDU="; // 검색API 승인키
...
%>
<script src="/resources/js/views/cubici/jquery-1.12.4.min.js"></script>
<script src="/resources/js/views/cubici/addrlink.js"></script>
<link rel="stylesheet" type="text/css" href="/resources/rudicks/mobile/css/maddrlink.css"></link>
...
<script language="javascript">
	...
	function makeList(xmlStr){
		...
		$("#searchContentBox").css("height","100%");
		pageMake(xmlStr);
	...
	function addrJuminRenew(idx){
		...
		if(docHeight > 300){ // 높이가 310인 경우 로고 위치 조정
	    	...
	    	$("#searchContentBox").css("100%");// 로고 위치 지정
	    }
	}
...
<body onload="init();" class ="visualSection"> 
	...
	<form name="AKCFrm" id="AKCFrm" method="post">
		...
		<div class="pop-address-search" style="width: 100%;" >
			...
			<div style="height: 100%;" id="searchContentBox">
				<fieldset>
					<legend>도로명주소 검색</legend>		
					<span class="wrap">
						<input type="text" class="popSearchInput" style="font-size: 14px; height:50px; font-weight: bold; ime-mode:active;" title="검색어를 입력하세요" name="keyword" id="keyword" placeholder="도로명주소, 건물명 또는 지번입력" value=""  tabindex="1"/>
						<input type="button" title="검색"  tabindex="2" onclick="javascript:$('#currentPage').val(1);  searchUrlJuso();" style=" cursor: pointer;"/>
					</span>
				</fieldset>
				<p class="guide" style="margin-top:10px; font-size:10px">검색어 예 : 도로명(반포대로 58), 건물명(독립기념관), 지번(삼성동 25)</p>
				<!-- nodata -->
		...
		<!-- 	<div class="logo" style="display: block;" >&nbsp;</div> -->
			...
			<div class="" style="display: block; position: relative; height: 100%; bottom: 0; background-color: #373737; text-align: center;">
				<a class="close" href="javascript:popClose();" title="창닫기" tabindex="3">돌아가기</a>
			</div>
		</div>
		</form>
</body>
