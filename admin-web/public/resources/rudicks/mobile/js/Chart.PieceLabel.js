 ...
 */
 (function () {
	...
	PieceLabel.prototype.drawDataset = function (dataset) {
		...
		for (var i = 0; i < meta.data.length; i++) {
			...
			if (this.arc) {
				...
				if (this.overlap) {
					...
					if (this.segment) {
						...
						if (start.x < end.x) {
							...
							end.x = position.x - metrics.width / 2 - 10;
							ctx.textAlign = 'left';
						} else {
							position.x = position.x - 20;
							end.x = position.x + metrics.width / 2 + 10;
				...
				if (drawable) {
					this.fillText(text, position, fontColor, 16);
					var label = element._chart.tooltip._data.labels[element._index];
					this.fillText(label, {x: position.x, y: position.y+20}, '#999', 12);
				}
			}
			ctx.restore();
		}
	...
	PieceLabel.prototype.fillText = function (text, position, fontColor, fontSize) {
		var ctx = this.ctx;
		if (typeof text === 'object') {
