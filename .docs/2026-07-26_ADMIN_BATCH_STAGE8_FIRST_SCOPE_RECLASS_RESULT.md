...
- 정산/상환 잔액 불일치 운영 데이터 반복 검수는 `2026-07-26_ADMIN_BATCH_STAGE9_FIRST_PHASE_CLOSURE_RESULT.md`에 1차 마무리 결과로 기록했다.
- legacy 세부 산식/정책 대조는 원천 batch/procedure 추가 확보 시 재검수한다.
- 실제 운영 데이터로 화면별 저장/수정/삭제 반복 검수와 미세 UI/레이아웃 보정은 운영검수 단계로 이관한다.
