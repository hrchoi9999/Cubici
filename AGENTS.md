...
- 한 번에 하나의 TASK와 하나의 batch만 수행한다.
- 사용자가 다음 batch를 승인하기 전에는 후속 batch를 시작하지 않는다.
- 조사, 구현, 검증이 분리 가능한 작업은 별도 작업 단위로 분리하되, 사용자 승인 범위를 넘지 않는다.
- 기존 Codex 세션과 `.codex` 아래 JSONL 파일은 읽지 않는다.
- 전체 diff, 대용량 로그, 대량 파일 내용은 대화창에 출력하지 않는다.
...
- 개별 명령, 검증, 서버 기동, smoke 실행이 10분 이내에 완료되지 않으면 같은 방식으로 계속 기다리지 않는다. 즉시 중단하거나 범위를 더 작게 나누고, 원인을 분리한 뒤 다른 방법으로 재시도해 최대한 빠르게 작업을 마무리한다.
- preview/dev 서버 기동과 Playwright smoke는 한 명령에 길게 묶지 않는다. 서버 기동, 포트 확인, route 1개 smoke, 서버 종료를 분리해서 수행한다.

## 작업공간 경계 원칙

- 작업 대상은 사용자가 명시한 현재 저장소와 원본 참고 경로로 제한한다.
- 원본 참고 경로는 사용자가 명시적으로 승인하지 않는 한 수정하지 않는다.
- 절대경로는 현재 작업 요청의 범위로만 해석하고, 저장소의 영구 지침으로 고정하지 않는다.
- 필요한 legacy 자료는 사용자가 명시한 원본 참고 경로 아래에 제공한 경우에만 참조한다.
...
- DB volume, 변환 CSV, 검증 산출물, 로그는 저장소 루트의 `data_local` 아래에만 둔다.
...
- 민감정보, 실제 `.env`, DB dump, backup SQL, service-account JSON은 `git add` 대상에 포함하지 않는다.
...
- 로컬 legacy 이력은 GitHub public push 기준으로 신뢰하지 않고, 최종 반영 시 sanitized 기준으로 정리한다.

## 개인정보/결제정보 원칙

- 기존 서비스 재현 목적의 내부 원본 데이터는 로컬 개발 PostgreSQL DB에서만 제한적으로 사용한다.
- 실제 개인정보, 결제 식별정보, 계좌정보, 카드정보는 코드, 문서, 로그, Git commit에 기록하지 않는다.
- 원본 DB 백업, 복원 DB, export 산출물은 사용자가 지정한 로컬 원본 또는 저장소 내부에서만 보관하고 외부 전송, 원격 업로드, 공개 배포를 금지한다.
