...
$AdminLogs = Join-Path $AdminWebRoot ".logs"
...
if (Test-LocalPort $AdminPort) {
  Write-Host "Admin web already listening: http://127.0.0.1:$AdminPort"
} else {
  $viteCli = Join-Path $AdminWebRoot "node_modules\vite\bin\vite.js"
  if (-not (Test-Path $viteCli)) {
    throw "Vite CLI not found: $viteCli"
  }
  $adminOut = Join-Path $AdminLogs "admin-$AdminPort.out.log"
  $adminErr = Join-Path $AdminLogs "admin-$AdminPort.err.log"
  $adminArgs = @(
    $viteCli,
    "--host", "127.0.0.1",
    "--port", "$AdminPort",
  ...
  $adminProcess = Start-Process -FilePath $NodeExe -ArgumentList $adminArgs -WorkingDirectory $AdminWebRoot -WindowStyle Hidden -RedirectStandardOutput $adminOut -RedirectStandardError $adminErr -PassThru
  $env:VITE_API_BASE_URL = $previousViteApiBaseUrl
  Write-Host "Admin web started: PID=$($adminProcess.Id), http://127.0.0.1:$AdminPort"
}

Write-Host "Admin URL: http://127.0.0.1:$AdminPort/admin/cubici/adminMonitor/fintech_trade"
