...
function repayChartView(barLabel, mbAccumContract, mbAccumRepay, mbBalance, mbBalancePercent){

	// 객체에 그래프 데이터가 쌓이는 것 방지하기 위해 객체 데이터 파괴
	if (mixedChart2 != undefined)
		mixedChart2.destroy();
	
	var ctx = document.getElementById('ac1p2-2-2').getContext('2d');
	
	var chartData = {
	    datasets: [{
	        label: '누적계약',
	        data: cumulative(mbAccumContract),
	        yAxisID: 'y-axis-1',
