...
$(document).ready(function() {
	...
	if ("${resultCode}" === "0") {
		...
		console.log("READY FUNC ERROR ::: "+"${resultCode}");
	}
        ...
        $.ajax({
			...
			success : function(result) {
				if (result.resultCode === 0) {
					...
					// modalInfo("ErrorCode 222 ::: " + result.resultCode);
					console.log("DATA SPREAD ERROR ::: "+result.resultCode);
				}
			},
...
function holidayStyle(holiday){  
	...
	for(let i=0; i<c_day.length; i++){
		c_day[i].parentNode.style.fontWeight = '500';
		if(c_day[i].parentNode.style.color == "rgb(51, 51, 51)"){ // 해당월 평일 : 색상으로 구분
			for(let j=0; j<holiday.length; j++){
				if(holiday[j] == c_day[i].innerText){
					document.getElementsByClassName("tui-full-calendar-weekday-grid-date")[i].style.color = '#ff4040';
					break;
				}
			}
		}
    ...
    <div class="calendarContents">
        ...
        <div class="contentArea">
            <div class="cal-top m-modalGrid">
                ...
                <li class="col-1"><a class="oiBtn excel" id="excelBtn">엑셀 다운로드</a></li>
                <div class="excelDiv"></div>
                </ul>
            </div>
            <div class="today">
                <button type="button" class="navBtn moveDay mBtn sColorLB rBtn" data-action="move-today">Today</button>
